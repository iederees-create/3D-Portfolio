...
export const projects: Project[] = [

  {
    ...
    galleryImages: [
      import.meta.env.BASE_URL + 'media/projects/hydro-clean-ct/02-desktop-homepage.png',
      import.meta.env.BASE_URL + 'media/projects/hydro-clean-ct/03-mobile-homepage.png',
      import.meta.env.BASE_URL + 'media/projects/hydro-clean-ct/04-quote-planner.png',
      import.meta.env.BASE_URL + 'media/projects/hydro-clean-ct/05-service-sections.png',
      import.meta.env.BASE_URL + 'media/projects/hydro-clean-ct/06-quote-summary.png',
      import.meta.env.BASE_URL + 'media/projects/hydro-clean-ct/07-whatsapp-email-handoff.png',
      import.meta.env.BASE_URL + 'media/projects/hydro-clean-ct/08-buyer-files.png',
      import.meta.env.BASE_URL + 'media/projects/hydro-clean-ct/09-customisation.png',
      import.meta.env.BASE_URL + 'media/projects/hydro-clean-ct/10-responsive-preview.png',
    ],
    galleryImageAlts: [
      'Hydro Clean responsive desktop homepage',
      'Hydro Clean mobile website view',
      'Hydro Clean surface cleaning request planner',
      'Hydro Clean editable exterior cleaning service sections',
      'Hydro Clean completed planner summary and estimate handoff',
      'Hydro Clean WhatsApp email copy and print handoff options',
      'Hydro Clean included buyer website files',
      'Hydro Clean site configuration and branding customisation',
      'Hydro Clean responsive desktop tablet and mobile preview',
    ],
    featured: true,
...

  {
    ...
    liveUrl: 'https://iederees-create.github.io/3D-Portfolio/?assistant=open',
  ...
  },
  {
    ...
    description: 'A Deriv partner launchpad with a public RSI Eclipse practice desk: fifteen volatility markets, six timeframes, and a scan grid anyone can watch on demo funds. Still a rebrandable affiliate funnel with risk disclosure — now with a live artefact instead of a stale screenshot.',
    tags: ['Affiliate Funnel', 'Live Demo Desk', 'RSI', 'React', 'TypeScript', 'Vite', 'Data Visualisation', 'Risk Disclosure'],
    liveUrl: 'https://iederees-create.github.io/deriv-affiliate-launchpad-template/lab',
    ...
    galleryImages: [
      `${import.meta.env.BASE_URL}projects/trading-affiliate/04-live-desk-desktop.png`,
      `${import.meta.env.BASE_URL}projects/trading-affiliate/05-live-desk-mobile.png`,
      `${import.meta.env.BASE_URL}projects/trading-affiliate/01-desktop.webp`,
      `${import.meta.env.BASE_URL}projects/trading-affiliate/02-mobile.webp`,
      `${import.meta.env.BASE_URL}projects/trading-affiliate/03-responsive.webp`,
    ],
    galleryImageAlts: [
      'Apex RSI Eclipse live desk on desktop with scan grid and gauges',
      'Apex RSI Eclipse live desk on a phone viewport',
      'Trading affiliate launchpad desktop website preview',
      'Trading affiliate launchpad mobile website preview',
      'Trading affiliate launchpad responsive preview',
    ],
    featured: true,
    toolBadge: 'Public RSI Eclipse Desk',
    toolHighlight: 'The live /lab board scans Volatility 10–250 (except Boom, Crash and Step) on 5m, 15m, 30m, 1h, 4h and daily charts. RSI(14) 20/80, one contract, 0.35 demo stake. Eclipse gauges and a heatmap so a visitor can see what the numbers mean. Not a signal room and not a profit claim.',
    mediaAlt: 'Apex RSI Eclipse live practice desk showing six timeframe gauges and a 15-market volatility scan grid',
    features: [
      'Public live practice desk at /lab — no login required to watch',
      'RSI Eclipse gauges plus a 15×6 heatmap of volatility markets and timeframes',
      'OR-scan across M5, M15, M30, H1, H4 and D1; fastest ready chart wins',
      'Boom, Crash and Step excluded; one contract at a time; 0.35 demo stake; no martingale',
      'Share controls and Open Graph cards on the desk, blog posts and project page',
      'Affiliate link manager controlled from src/config/affiliateConfig.ts',
      'Dedicated risk disclosure, affiliate disclosure, and demo-funds labelling throughout',
      'Gated MT5 Expert Advisor for verified partner downline — visitor scripts are never executed',
    ],
    caseStudy: {
      methodology: 'Started as a static GitHub Pages funnel, then grew a public Deriv demo engine so visitors could watch a rule instead of a cropped screenshot. The current artefact is Apex RSI Eclipse: Wilder RSI(14) 20/80 across fifteen volatility indices and six timeframes, rendered as eclipse dials and a scan grid with plain-language captions.',
      toolsUsed: ['Vite', 'React', 'TypeScript', 'Node.js', 'Deriv WebSocket', 'GitHub Pages', 'Render', 'Open Graph / Twitter cards'],
      technicalChallenge: {
        title: 'Showing a multi-market scan without turning the page into a signal room',
        body: 'A 15×6 RSI grid is unreadable as a number dump, and a 5-minute-only board looked idle. The UI treats RSI as an instrument: buy/sell bands on dials, a heatmap with numbers plus shape, and three “how to read this” cards. Options cannot attach CFD stops, so the site uses time-based expiry while the gated MT5 pack keeps 1,000/25,000 point stops. Demo token and MT5 password never appear in the public payload.',
      },
      privacyDesign: 'The public board fetches a redacted live payload. Demo tokens, script text and trade passwords stay server-side. Lead capture remains WhatsApp and email. Affiliate links come from local config.',
      testResults: [
        'Public /api/lab/live returns scannedSymbols, timeframes and rsiByMarket without secrets',
        'Launchpad production build and internal link check pass',
        'Live GitHub Pages lab route and portfolio article are shareable with OG images',
      ],
    },
  },
  {
    ...
    tags: ['React', 'TypeScript', 'Python', 'Pandas', 'D3', 'Data Visualisation', 'Machine Learning', 'Analytics', 'Apache Airflow', 'Data Engineering'],
    ...
    galleryImages: [
      ...
      `${import.meta.env.BASE_URL}projects/insightforge/11-pipeline-dashboard-output.webp`,
      `${import.meta.env.BASE_URL}projects/insightforge/12-pipeline-data-quality.webp`,
      `${import.meta.env.BASE_URL}projects/insightforge/13-pipeline-architecture-dag.webp`,
    ],
    galleryImageAlts: [
      ...
      'Business Performance Dashboard loaded with real output from the optional Apache Airflow pipeline, showing matching revenue and order KPIs computed independently by both the pipeline and the browser',
      'Data Quality Lab independently re-detecting the same data-quality issues the Airflow pipeline already found and reported in its own quality report',
      'Airflow ETL pipeline task graph — discover_inputs, validate_schema, clean_and_deduplicate, join_datasets, calculate_metrics, export_dashboard_data and generate_quality_report — rendered directly from the live DAG object',
    ],
    features: [
      ...
      'Optional Apache Airflow ETL pipeline (advanced/developer feature) — automated CSV discovery, schema validation, cleaning and deduplication, dataset joining, KPI calculation and dashboard-ready export, with retries and a data-quality report; entirely separate from and unnecessary for the core browser dashboard',
    ],
    caseStudy: {
      methodology: 'Built as a fully client-only, single-page web application — all CSV parsing, analysis and visualisation happen in the visitor\'s own browser. There is no backend server and no database; the app ships as a static bundle (React + TypeScript + Vite) that runs entirely on the user\'s device. The product later grew a second, entirely optional half: an Apache Airflow ETL pipeline (Python, TaskFlow API) for buyers who want repeatable, scripted data preparation instead of a manual browser upload. The DAG is deliberately thin — every task is a small adapter around a pure, independently-testable Python function (discover → validate → clean/dedupe → join → calculate KPIs → export → quality report) — and those same functions are reused verbatim by a standalone CLI, so there is exactly one implementation of the logic, not two. Runs are idempotent by construction: deterministic output paths keyed by logical date, atomic file writes, and no runtime randomness, verified by a test that runs the pipeline twice and diffs every output file byte-for-byte.',
      toolsUsed: ['React', 'TypeScript', 'Vite', 'Zustand', 'D3', 'Papa Parse', 'Tailwind CSS', 'Python (optional toolkit)', 'Pandas (optional toolkit)', 'scikit-learn (optional toolkit)', 'Apache Airflow (optional pipeline)'],
      technicalChallenge: {
      ...
      privacyDesign: 'All processing — CSV parsing, segmentation, sentiment analysis, forecasting and report generation — happens locally in the browser. No sales, customer or review data is ever uploaded to a server. The only thing persisted between sessions is the user\'s column-mapping preference, and it is only stored after the user explicitly opts in. The optional Airflow pipeline extends the same principle to automated batch runs: every input, intermediate, and output file stays on the user\'s own machine, no credentials or API keys are required anywhere in the pipeline, and no external service is ever called.',
      testResults: [
        ...
        '30 pytest tests passing in the new Airflow pipeline (unit tests per transform module, a DAG-import/structure test, a full synthetic end-to-end run, and an XCom-payload size guard)',
        'Full DAG executed successfully via `airflow dags test`, all 7 tasks completing in dependency order with real inter-task data passed through XCom',
        'Pipeline rerun for the same logical date produced byte-for-byte identical output on a second run, verifying idempotency',
        'Pipeline-generated dashboard export loaded into the live app auto-mapped on every column and reproduced the exact same headline KPIs (revenue, order count) the pipeline itself computed independently',
      ],
    },
  },
  {
  ...
  },
  {
    ...
    liveUrl: 'https://www.raversus.com',
